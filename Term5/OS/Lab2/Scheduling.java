...
public class Scheduling {
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      String numberFormat = "%9s";
      String processFormat = "%12s%15s%17s%15s%n";

      out.printf(numberFormat + processFormat,
          "Process #", "CPU Time", "IO Blocking",
          "CPU Completed", "CPU Blocked");

      for (i = 0; i < processVector.size(); i++) {
        sProcess process = (sProcess) processVector.elementAt(i);

        out.printf(numberFormat, i);
        out.print(process.format(processFormat));
      }
