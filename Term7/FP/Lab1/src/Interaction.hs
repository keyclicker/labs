...
import qualified Resources as RT ( ResourcesInfo( ResourcesInfo, tableName, fieldNames, ids, names, types, annotations, links, purposes, openDate, usageTime, rules, statistics ), emptyResourceStruct )
import qualified AuthorsResources as A2RT ( AuthorToResources ( AuthorToResources, tableName, fieldNames, ids, authorIds, resourceIds ), emptyAuthorToResourceStruct, getAuthorId, getResourceId )
import qualified UsersResources as U2RT ( UserToResources ( UserToResources, tableName, fieldNames, ids, userIds, resourceIds, usageStart ), emptyUserToResourceStruct, getUserId )
import qualified Users as UT ( UsersInfo( UsersInfo, tableName, fieldNames, ids, emails, passwords, names ), emptyUserStruct )
import qualified Authors as AT ( AuthorsInfo( AuthorsInfo, tableName, fieldNames, ids, emails, names, surnames ), emptyAuthorStruct )
import qualified Converter as Conv ( readInt, readStr, strToDay, strToInt32, strToDayUserInteract, myToInt32 )
import qualified Connector as MSQLC ( showTables, Table (addValue, getAllValues, printInfo, deleteValue, updateKeyField, updateField, getValue, drop) )
